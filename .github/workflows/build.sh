...
if test -f "${BUILDCACHE}/build_hash_mfadmin_${BRANCH}_`cat .build_hash`"; then
...
rm -Rf html_doc
if test -d /opt/metwork-mfadmin-${TARGET_DIR}/html_doc; then cp -Rf /opt/metwork-mfadmin-${TARGET_DIR}/html_doc . ; fi
...
rm -f ${BUILDCACHE}/build_hash_mfadmin_${BRANCH}_*
hash_file=${BUILDCACHE}/build_hash_mfadmin_${BRANCH}_`cat .build_hash`
touch ${hash_file}
rm -f ./build_hash
